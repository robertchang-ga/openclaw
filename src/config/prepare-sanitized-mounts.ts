...
export async function prepareSanitizedMounts(opts?: {
  ...
  if (fs.existsSync(agentsDir)) {
    ...
    for (const agentId of agentIds) {
      ...
      // Models registry (read-write, generated by ensureOpenClawModelsJson at runtime)
      // Contains custom provider definitions (e.g. google-antigravity) — no secrets.
      // API keys are resolved via the secrets proxy, not stored in models.json.
      const modelsJsonPath = path.join(agentAgentDir, "models.json");
